...
#### Running Without a Database (In-Memory Mode)
...
dotnet run --project src/PerfectFit.Web --launch-profile inmemory
```

**Note**: In-memory mode uses `appsettings.InMemory.json` configuration. Data is stored in memory and will be lost when the application restarts. This mode is useful for:
